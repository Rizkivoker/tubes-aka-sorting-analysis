...
def draw_bars(area, data, active, final_idx, active_color):
    # Gunakan float untuk presisi posisi agar tidak menumpuk di satu sisi
    n = len(data)
    bar_w = area.width / n 

    for i, v in enumerate(data):
        # Posisi X menggunakan float agar distribusi merata
        x = area.x + (i * bar_w)
        
        # Penyesuaian tinggi agar tetap proporsional dengan layar
        ...
        # Gunakan max(1, ...) agar lebar batang minimal 1 pixel
        # Hilangkan "- 1" jika n > 200 agar tidak hilang karena spasi
...
def draw_input(text):
    ...
    text_tugas = font.render("Tugas Besar Analisis Kompleksitas Algoritma", True, (255, 215, 0))
    text_kelompok = font.render("Kelompok 4 - IF 48-12 - Telkom University", True, (255, 215, 0))
    ...
    title = font.render("MERGE SORT VISUALIZATION APP (RECURSIVE vs ITERATIVE)", True, (0, 255, 255))
